        ...
        <div class="flex flex-col justify-between space-y-3 px-2">
            ...
            <div class="flex items-center gap-4">
                <img class="h-10 w-10 overflow-hidden rounded-full bg-zinc-300 object-cover text-[0]"
                     src="{{ $post->user->avatar }}" alt="{{ $post->user->getName() }}">
                <div>
                    <span title="{{ $post->user->getName() }}"
                          class="block max-w-[150px] overflow-hidden text-ellipsis whitespace-nowrap text-sm font-semibold">{{ $post->user->getName() }}</span>
                    <span
